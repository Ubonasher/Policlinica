...
namespace Policlinica.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            if (ModelState.IsValid && WebSecurity.Login(model.UserName, model.UserName, persistCookie: model.RememberMe))
            {
        ...
        //
        // GET: /Account/LoginAdmV

        [AllowAnonymous]
        public ActionResult LoginAdmV(string returnUrl)
        {
            ViewBag.ReturnUrl = returnUrl;
            return View();
        }
        ...
        //
        // POST: /Account/LoginAdmV

        [HttpPost]
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public ActionResult LoginAdmV(LoginModelAdmV model, string returnUrl)
        {
        ...
        private BdEntities db1 = new BdEntities();

        [AllowAnonymous]
        public ActionResult Register()
        {
            ViewBag.Отделение = new SelectList(db1.Otdelenie, "Id", "Название_отделени");
            return View();
        }
        ...
        public ActionResult Register(RegisterModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    System.Web.Security.Roles.AddUserToRole(model.UserName, "v");
                    return RedirectToAction("Index", "Vrachi");
                }
            ...
            // Появление этого сообщения означает наличие ошибки; повторное отображение формы
            ViewBag.Отделение = new SelectList(db1.Otdelenie, "Id", "Название_отделени", model.Отделение);
            return View(model);
        }
